...
export class OpenAIService {
  ...
  async generateQuiz(request: IGenerateQuizRequest): Promise<Question[]> {
    try {
      ...
      return this.formatQuestions(generatedData.questions, request);
    } catch (error) {
...
Response Format:
{
  ...
  private buildPrompt(request: IGenerateQuizRequest): string {
    const {
      academicLevel,
      subject,
      topic,
...
Academic Level: ${this.formatAcademicLevel(academicLevel)}
Subject: ${subject}
Topic: ${topic}
Language: ${language}
...
Question Count: ${questionCount}`;
  ...
  private formatQuestions(questions: any[], request: IGenerateQuizRequest): Question[] {
    return questions.map((q, index) => ({
      id: `q_${Date.now()}_${index}`,
      question: q.question,
      type: q.type === 'mixed' ? this.getRandomQuestionType() : (q.type || request.questionType),
      options: q.options,
