...
class QuestionService {
  async createQuestion(questionData: ICreateQuestionRequest, userId: string): Promise<IQuestion> {
  ...
  async getQuestionById(questionId: string): Promise<IQuestion> {
    const question = await Question.findById(questionId).populate('createdBy', 'profile.fullName email');
  ...
  async updateQuestion(
    ...
    if (!isAdmin && question.createdBy.toString() !== userId) {
      throw new ApiError(StatusCodes.FORBIDDEN, 'You can only update your own questions');
    }
  ...
  async deleteQuestion(questionId: string, userId: string, isAdmin: boolean = false): Promise<void> {
    const question = await Question.findById(questionId);
    ...
    if (!isAdmin && question.createdBy.toString() !== userId) {
      throw new ApiError(StatusCodes.FORBIDDEN, 'You can only delete your own questions');
    }
  ...
  async getUserQuestions(
    ...
    return await Question.paginate(filter, {
      ...options,
      sort: options.sortBy ? { [options.sortBy]: options.sortOrder === 'desc' ? -1 : 1 } : { createdAt: -1 },
    });
  }

  async getApprovedQuestions(options: IPaginateOptions): Promise<IPaginateResult<IQuestion>> {
    const filter = { isApproved: true };
    return await Question.paginate(filter, {
      ...options,
      sort: options.sortBy ? { [options.sortBy]: options.sortOrder === 'desc' ? -1 : 1 } : { usageCount: -1 },
  ...
  async searchQuestions(
    ...
    return await Question.paginate(query, {
      ...options,
      sort: options.sortBy ? { [options.sortBy]: options.sortOrder === 'desc' ? -1 : 1 } : { usageCount: -1 },
      populate: [{ path: 'createdBy', select: 'profile.fullName' }],
    });
  ...
  async approveQuestion(questionId: string, isApproved: boolean): Promise<IQuestion> {
    const question = await Question.findById(questionId);
  ...
  async getQuestionsBySubject(subject: string): Promise<IQuestion[]> {
  ...
  async generateQuestion(
    ...
    try {
      const generatedQuestion = await openAIService.generateSingleQuestion(
        subject,
        topic,
        academicLevel,
        difficulty,
  ...
  async improveQuestion(
    ...
    if (question.createdBy.toString() !== userId) {
      throw new ApiError(StatusCodes.FORBIDDEN, 'You can only improve your own questions');
    }

    try {
      const improvedQuestion = await openAIService.improveQuestion(
        {
